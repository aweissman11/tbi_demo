...
import { AlertBanner } from "../interactive/AlertBanner";
 ...
 * @param sticky - if true, the navbar will be sticky to the top of the page.
 * @param banner - a banner to display at the top of the navbar.
 * @param slot - a slot to display at the bottom of the navbar.
 * @param children - a slot to display at the bottom of the navbar.
 */
export function Nav({ sticky = false, banner, slot, children }: NavProps) {
  ...
  const links = [
    ...
    { name: t("Search"), href: "/contentful/search" },
  ];
  ...
  return (
    <Popover
      as="nav"
      id="nav-header"
      className={clsx(sticky && "sticky top-0 z-50")}
    >
      {banner && <AlertBanner content={banner} />}
      <div className="border-1 flex h-[80px] items-center justify-between border border-border-weak bg-white px-10">
